...
m9 <- ivreg(logforemprate~logyouthshare + logLhat + factor(year)+factor(lmr_id)|.-logyouthshare+ logLhat+loginstrument, data = EmpData_EastGermany, weights=popshare) 
m10 <- ivreg(loggeremprate~logyouthshare + logLhat + factor(year)+factor(lmr_id)|.-logyouthshare+ logLhat+loginstrument, data = EmpData_EastGermany, weights=popshare) 
...
m11 <- ivreg(logemprate2024~logyouthshare + logLhat + factor(year)+factor(lmr_id)|.-logyouthshare+ logLhat+loginstrument, data = EmpData_EastGermany, weights=popshare) 
m12 <- ivreg(logemprate2534~logyouthshare + logLhat + factor(year)+factor(lmr_id)|.-logyouthshare+ logLhat+loginstrument, data = EmpData_EastGermany, weights=popshare) 
m13 <- ivreg(logemprate3544~logyouthshare + logLhat + factor(year)+factor(lmr_id)|.-logyouthshare+ logLhat+loginstrument, data = EmpData_EastGermany, weights=popshare) 
m14 <- ivreg(logemprate4554~logyouthshare + logLhat + factor(year)+factor(lmr_id)|.-logyouthshare+ logLhat+loginstrument, data = EmpData_EastGermany, weights=popshare) 
m14a<- ivreg(logemprate5564~logyouthshare + logLhat + factor(year)+factor(lmr_id)|.-logyouthshare+ logLhat+loginstrument, data = EmpData_EastGermany, weights=popshare) 
...
# Robustness Checks 2: Role of Youngest Cohort and Excl University Towns
...
robust_se_m35 <- sqrt(diag(vcovHC(m33, type = "HC3")))
robust_se_m36 <- sqrt(diag(vcovHC(m34, type = "HC3")))
coefm35 <- coeftest(m35, vcovHC(m35,"HC3"))
coefm36 <- coeftest(m36, vcovHC(m36,"HC3"))

#First we exclude the regions where the major economic activity is driven by universities
...
mcheck <- lm(logempratetransform~0+logyouthsharetransform+logLhattransform+factor(year), data = EmpData_EastGermany)
f1 <- flexmix(.~x|lmr_id, model = FLXMRglmfix(formula = logempratetransform~logyouthsharetransform, 
                                              fixed=~logLhattransform+factor(year)), data = EmpData_EastGermany, k = 1)
f2 <- stepFlexmix(.~x|lmr_id, model = FLXMRglmfix(formula = logempratetransform~0+logyouthsharetransform, 
                                                  fixed=~logLhattransform+factor(year)), data = EmpData_EastGermany, k=2:8, nrep=3)
...
EmpData_EastGermany$Cluster <- clusters(f)
f <- refit(f)
Nocluster <- f@k
Probs$lmr_id <- EmpData_EastGermany$lmr_id
clustermean <- EmpData_EastGermany %>% group_by(lmr_id) %>% 
...
RegionData <- filter(EmpData_EastGermany,year==2010) # Just pick one year
...
# To see other color schemes run: RColorBrewer::display.brewer.all()
