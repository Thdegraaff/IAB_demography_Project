...
DESCRIPTION
***********
Compared to ..._160419_001 this file estimates a model in which the dependent 
variable and the youth-share variable are not in logs.
*******************************************************************************/
...
foreach word of local words {
	...
	regress unemp_rate`word' pop_share`word' i.year i.lmr_id, ///
	vce(robust)
		
	outreg2 using ${anal}/Analysis_Demo_`filedate'_`filenum'.xls, ///
	excel append dec(5) ///
	ctitle("unemp_rate`word'", "OLS", "robust") ///
	sortvar(pop_share_20_24 pop_share_18_24 pop_share_15_24)
		
	ivregress 2sls unemp_rate`word' i.year i.lmr_id ///
	(pop_share`word' = pop`word'_ins2), ///
	vce(robust)
	
	outreg2 using ${anal}/Analysis_Demo_`filedate'_`filenum'.xls, ///
	excel append dec(5) ///
	ctitle("unemp_rate`word'", "2SLS", "robust") ///
	sortvar(pop_share_20_24 pop_share_18_24 pop_share_15_24)
	
	regress emp_rate`word' pop_share`word' i.year i.lmr_id, ///
	vce(robust)
		
	outreg2 using ${anal}/Analysis_Demo_`filedate'_`filenum'.xls, ///
	excel append dec(5) ///
	ctitle("emp_rate`word'", "OLS", "robust") ///
	sortvar(pop_share_20_24 pop_share_18_24 pop_share_15_24)
		
	ivregress 2sls emp_rate`word' i.year i.lmr_id ///
	(pop_share`word' = pop`word'_ins2), ///
	vce(robust)
	
	outreg2 using ${anal}/Analysis_Demo_`filedate'_`filenum'.xls, ///
	excel append dec(5) ///
	ctitle("emp_rate`word'", "2SLS", "robust") ///
	sortvar(pop_share_20_24 pop_share_18_24 pop_share_15_24)
